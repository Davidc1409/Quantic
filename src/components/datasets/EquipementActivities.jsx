...
export function EquipementActivities ({name, address, type, paidService, zipCode, targetCollapse,cityDistrictList}) {
    ...
    return (
        <div className="card shadow-sm p-3 mb-5 bg-body-tertiary rounded">
            <div className="card-body">
                ...
                <p className="nexaLight">Code postal : {zipCode} Paris</p>
                <p className="nexaLight">Type : {type}</p>
                <p className="nexaLight">Payant : {paidService}</p>
            </div>
            <p className="d-inline-flex gap-1">
                    <button className="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target={"#"+targetCollapse} aria-expanded="false" aria-controls="collapseExample" onClick={(e)=>{setCollapseValue(e.target.ariaExpanded)}}>
